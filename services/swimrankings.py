...
class SessionManager:
    ...
    def check_request_rate_limit(self):
        ...
        valid_requests = [
            t for t in self.request_history if t > window_start_time]
        ...
        if len(valid_requests) >= self.max_requests_per_second:
            ...
            self.request_history = [
                t for t in self.request_history if t > window_start_time + time_to_wait]
...
class Athlete(ScraperMixin):
    ...
    def list_personal_bests(self, season="-1") -> list:
        ...
        """
        params = {'page': 'athleteDetail',
                  'athleteId': self.athlete_id, 'pbest': season}
        try:
        ...
        for row in table.find_all('tr', {'class': ['athleteBest0', 'athleteBest1']}):
            ...
            # Long course or short course pool
            course_cell = row.find('td', {'class': 'course'})
            course_length = course_cell.get_text(strip=True)

            # Time
            time_cell = row.find(
                'td', {'class': ['time', 'swimtimeImportant']})
            time = time_cell.get_text(strip=True)

            result_url = time_cell.find('a')['href']
            result_id = int(parse_qs(urlparse(result_url).query)['id'][0])

            # FINA Points for the result
            fina_points = row.find(
                'td', {'class': 'code'}).get_text(strip=True)
            
            # Date of the PB
            pb_date = row.find('td', {'class': 'date'}).get_text(strip=True)

            # City or Country of the PB
            city = row.find('td', {'class': 'city'}).get_text(strip=True)

            # Name of the Meet
            meet_cell = row.find('td', {'class': 'name'})
            meet_url = meet_cell.find('a')
            meet_text = meet_url.get_text(strip=True)
            meet_title = meet_url.get('title')  # This will return None if 'title' attribute is not present

            meet = meet_text if meet_title is None else meet_title

            data.append({'result_id': result_id, 'event_name': event_name,
                        'course_length': course_length, 'time': time, 'fina_points': fina_points,
                        'date': pb_date, 'city': city, 'meet_name': meet})
        return data
    ...
    def list_meets(self) -> list:
        ...
        """
        params = {'page': 'athleteDetail',
                  'athleteId': self.athlete_id, 'athletePage': 'MEET'}
        try:
            soup = self._get_page_content(params)
        ...
        for row in table.find_all('tr', {'class': ['athleteMeet0', 'athleteMeet1']}):
            ...
            meet_date = unicodedata.normalize(
                "NFKD", date_cell.get_text(strip=True))
            ...
            data.append({'meet_id': meet_id, 'meet_date': meet_date,
                        'meet_city': meet_city, 'meet_name': meet_name})
        return data
...
class Meet(ScraperMixin):
    ...
    def list_events(self):
        ...
        menu = table.find(lambda tag: tag.name ==
                          'td' and tag['class'][0] == 'navigation' and search_text in tag.text)
        for item in menu.find_all('option'):
            if item['value'] != "0":
                data.append(
                    {'event_id': item['value'], 'event_gender': '1', 'event_name': item.get_text(strip=True)})
        # Add Women's events
        ...
        for item in menu.find_all('option'):
            if item['value'] != "0":
                data.append(
                    {'event_id': item['value'], 'event_gender': '2', 'event_name': item.get_text(strip=True)})
        return data
    ...
    def list_races(self, event_id, gender):
        ...
        """
        params = {'page': 'meetDetail', 'meetId': self.meet_id,
                  'gender': gender, 'styleId': event_id}
        try:
            soup = self._get_page_content(params)
        ...
        for (id, table) in enumerate(tables):
            ...
            name = unicodedata.normalize(
                "NFKD", name_cell.get_text(strip=True))
    ...
    def list_results(self, event_id, gender, race_id):
        ...
        for row in tables[race_id-1].find_all('tr', {'class': ['meetResult0', 'meetResult1']}):
            ...
            results.append({'result_id': result_id, 'athlete_id': athlete_id, 'name': name,
                           'club_name': club_name, 'time': time, 'split_times': split_times})
        return results
...
class Meets(ScraperMixin):
    ...
    def list_time_periods(self):
        ...
        """
        params = {'page': 'meetSelect',
                  'nationId': '0', 'selectPage': 'RECENT'}
        try:
            soup = self._get_page_content(params)
        ...
        for item in menu.find_all('option'):
            if item['value'] != "RECENT" and item['value'] != "BYTYPE":
                periods.append({'period_id': item['value'], 'period_name': unicodedata.normalize(
                    "NFKD", item.get_text(strip=True))})
        return periods
    ...
    def list_nations(self):
        ...
        for item in menu.find_all('option'):
            if item['value'] != "$$$":
                nations.append({'nation_id': item['value'], 'nation_name': unicodedata.normalize(
                    "NFKD", item.get_text(strip=True))})
        return nations
    ...
    def list_meets(self, nation_id=None, time_period_id='RECENT'):
        ...
        - list: A list of dictionaries containing information about each meet.
        """
        params = {'page': 'meetSelect', 'nationId': nation_id,
                  'selectPage': time_period_id}
        try:
            soup = self._get_page_content(params)
        ...
        for table in tables:
            for row in table.find_all('tr', {'class': ['meetSearch0', 'meetSearch1']}):
                ...
                meet_date = unicodedata.normalize(
                    "NFKD", date_cell.get_text(strip=True))
                city_cell = row.find('td', {'class': 'city'})
                meet_city = unicodedata.normalize(
                    "NFKD", city_cell.find('a').get_text(strip=True))
                meet_url = city_cell.find('a')['href']
                meet_name = row.find_all('td', {'class': 'name'})[
                    1].find('a').get_text(strip=True)
                course_cell = row.find('td', {'class': 'course'})
                course_length = course_cell.get_text(strip=True)
                meet_id = parse_qs(urlparse(meet_url).query)['meetId'][0]
                meets.append({'meet_id': meet_id, 'meet_date': meet_date, 'meet_city': meet_city,
                             'meet_name': meet_name, 'course_length': course_length})
        return meets
...
class Club(ScraperMixin):
    ...
    def list_athletes(self, gender=0):
        ...
        params = {'page': 'rankingDetail', 'clubId': self.club_id,
                  'stroke': '9', 'athleteGender': athlete_gender}
        try:
            soup = self._get_page_content(params)
        ...
        for table in tables:
            for row in table.find_all('tr', {'class': ['athleteSearch0', 'athleteSearch1']}):
                ...
                athlete_id = parse_qs(urlparse(athlete_url).query)[
                    'athleteId'][0]
...
if __name__ == '__main__':
    # Example usage
    scraper = SwimrankingsScraper()
    athelete = scraper.get_athlete('5478194')
